...
Ext.onReady(function(){
			...
			items:[{
				...
				items:[{
					...
					items:[{
						...
						text:'查询',
						handler:function(){				
						var f = djmxForm.getForm();//store.reload({params:{condition1:param1,condition2:param2}});	
				...
				},
				{
					...
					items:[{
						...
						if (f.isValid()) {
							...
							//for(var j=0;j<kcspStore.getCount();j++)
							//djmxGrid.view.refresh();
								...
								}//djmxStore.reload({params:f.getValues()});
						}
					}
					}]
				},
	...
	//var spid = DjxmObj["spid"];//. ckdForm.getForm().findField("gh").getValue();
	//var index = kcspStore.find('spid',spid);
				//alert(index);
	//kcspStore.find('kcsl',kcsl);
	//djmxStore.add(kcspStore({name: 'kcsl',sex: 0}));
    ...
    var djmxGrid = new Ext.grid.GridPanel({
        ...
        stripeRows: true, 	//行分隔符store,records,options
        columnLines : true, //列分隔符djmxGrid.getStore().addListener({
        margins:'10 20 0 20',
